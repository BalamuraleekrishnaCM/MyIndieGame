...
public class MiniGameCollection : MonoBehaviour
{
    ...
    void Clear(){foreach(var x in items)if(x && x!=title.gameObject && x!=info.gameObject)Destroy(x);items.RemoveAll(x=>x==null || x!=title.gameObject&&x!=info.gameObject);}
    ...
    void Colors(){int wanted=Random.Range(0,4);string[] c={"RED","GREEN","BLUE","YELLOW"};title.text="MATCH "+c[wanted];for(int i=0;i<4;i++){int x=i;var b=Btn(c[i],()=>{score+=x==wanted?2:-1;Colors();UpdateInfo();},.3f,.12f,.3f+(i%2)*.4f,.6f-(i/2)*.23f);b.GetComponent<Image>().color=colors[i];}}
    ...
    void Memory(){for(int i=0;i<8;i++){Btn("?",()=>{score++;UpdateInfo();if(score>=8)title.text="ALL PAIRS FOUND!";},.2f,.12f,.27f+(i%4)*.15f,.62f-(i/4)*.2f);}}
    ...
    void LateUpdate(){if(mode<0)return;time-=Time.deltaTime;if(time<=0){int finished=mode;time=0;mode=-1;Clear();title.text="TIME UP!";info.text="Final score: "+score;Btn("RETRY",()=>Play(finished),.45f,.1f,.5f,.4f);Btn("HOME",Home,.3f,.08f,.5f,.27f);}else UpdateInfo();}
}