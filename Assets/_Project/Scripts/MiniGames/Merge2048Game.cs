...
{
 public sealed class Merge2048Game:MonoBehaviour,IMiniGame
 {
  const string Id="merge-2048"; const int Index=9; MiniGameSession session; bool initialized;
  ...
  public void MergeTiles(int points=1){if(IsRunning&&points>0)session.AddScore(points);} public void CompleteGame(){if(IsRunning)session.Complete();}
 }
}