...
{
 public sealed class BallSortGame:MonoBehaviour,IMiniGame
 {
  const string Id="ball-sort"; const int Index=7; MiniGameSession session; bool initialized;
  ...
  public void SortBall(bool correct,int points=1){if(IsRunning&&correct&&points>0)session.AddScore(points);} public void CompleteGame(){if(IsRunning)session.Complete();}
 }
}